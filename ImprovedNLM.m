...
% 波参数使去噪性能达到最佳
% 相对于 ONLM 的指数型核函数，高斯核函数的响应曲线在像素邻域距离较大时更为平坦，随着距离的增大而迅速下降；而余弦核函数的响应曲线
% 在整个区域都比较平坦；改进的余弦高斯型核函数综合了它们的优点，在距离较小时有更大的加权，而当距离增大时权值随之下降，
       ...
       for similarBox_i=pixel_i-searchBox_r+similarBox_r:pixel_i+searchBox_r-similarBox_r
           for similarBox_j=pixel_j-searchBox_r+similarBox_r:pixel_j+searchBox_r-similarBox_r       
               ...
               if dij>h2
                   ...
                   fk=exp(-dij/(h1^2))*cos(pi*dij/(2*h2));  
               end
...
title('ImprovedNLM denoised image');
